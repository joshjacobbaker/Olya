...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    ...
    const data = req.body
    // console.log(data)
    const dataReadResponse = await fs.readFile(path.join(process.cwd(), "./src/data/reactFormData.json"), { encoding: "utf8" })
    ...
    dataReadResponseParsed.push(data)
    console.log(dataReadResponseParsed)
    const dataWriteResponse = await fs.writeFile(path.join(process.cwd(), "./src/data/reactFormData.json"), JSON.stringify(dataReadResponseParsed))
